...
class FeatureExtractor:
    ...
    def get_parser(self):
        ...
        parser.add_argument(
            "--image_dir", type=str, help="Image directory or file"
        )
        parser.add_argument(
            "--feature_name", type=str, help="The name of the feature to extract", default="fc6",
        )
        parser.add_argument(
    ...
    def _process_feature_extraction(
        ...
        n_boxes_per_image = [len(boxes) for boxes in output[0]["proposals"]]# [100]
        # output[0]["scores"].shape:  torch.Size([100, 1601])
        ...
        for i in range(batch_size):
            ...
            scores = score_list[i]
            max_conf = torch.zeros((scores.shape[0])).to(cur_device) # 100
            conf_thresh_tensor = torch.full_like(max_conf, conf_thresh)
            start_index = 1
            # Column 0 of the scores matrix is for the background class
            if self.args.background:
                start_index = 0
            for cls_ind in range(start_index, scores.shape[1]): # for each category in 1601 categories
                # scores is 100*1601
                # cls_scores is 100*1
            ...
            # feats[i] 100*2048
            # we do not need the post processing of roi_head, only the original fc6 features are needed
            ...
            info_list.append(
                {
                    ...
                    # we don't need the object predicted 
                    # as we only want features
    ...
    def _save_feature(self, file_name, feature, info):
        ...
        # if os.path.exists(os.path.join(self.args.output_folder, file_base_name)) and \
        # os.path.exists(os.path.join(self.args.output_folder, info_file_base_name)):
        #     return
        np.save(
